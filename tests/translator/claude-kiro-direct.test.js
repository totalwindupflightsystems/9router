...
const systemPromptOf = (out) => {
  const msg0 = (out.conversationState?.history || []).find((m) => m.userInputMessage)?.userInputMessage.content;
  return msg0 || out.conversationState?.currentMessage?.userInputMessage?.content || "";
};
...
describe("Claude → Kiro (direct route)", () => {
  ...
  it("folds the Claude system instruction into the session-start user message (upstream parity)", () => {
    const out = C2K({
  ...
  it("keeps the session-start system instruction stable across turns", () => {
    const first = C2K({
    ...
    // Upstream parity: the system instruction rides in the session-start
    // user message — identical across turns (modulo the per-turn time
    // context, which stays fresh for every request).
    expect(systemPromptOf(first).split("[Context:")[0])
      .toBe(systemPromptOf(second).split("[Context:")[0]);
