...
| `FED-GAP-04` | Automated check for gateway health / free-OpenCode completion (replaces manual `dogfood-ok`) | A runnable, network-free check asserts the real `GET /api/health` returns `{ok:true}` and the free-tier completion path returns a terminal `[DONE]` stream |
